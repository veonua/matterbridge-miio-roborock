import { IDeviceState, IErrorState, IMiCommand, IMiResponse, ISuccessState } from '../types.js';
...
export abstract class IMiDevice<Command, State extends ISuccessState> {
  ...
  get needsHandshake() {
    return this.packet.needsHandshake;
  }
  packet: Packet;
  // type: string;
  // deviceId: number;
  ...
  get cachedQuery() {
    if (Date.now() - this.lastQueryResponse > 10 * 1000) {
      return null;
    }
  ...
  async sendQuery(deviceManager: smarthome.DeviceManager, requestId: string): Promise<IDeviceState> {
    try {
      const resp = await this.sendMi(deviceManager, requestId, this.convertImpl(smarthome.Intents.QUERY, undefined));
  ...
  onResponse(requestId: string, command: string, result: smarthome.DataFlow.UdpResponseData): IDeviceState {
    const resp = this.packet.decode(result.udpResponse.responsePackets);
  ...
  makeRequest(requestId: string, command: string, params: Command | undefined): smarthome.DataFlow.UdpRequestData {
    return this.makeCommandRequest(requestId, this.convertImpl(command, params));
  }

  makeCommandRequest(requestId: string, command: IMiCommand): smarthome.DataFlow.UdpRequestData {
    const req = new smarthome.DataFlow.UdpRequestData();
    req.requestId = requestId;
    req.deviceId = this.deviceIdString;
    req.port = portNumber;
    const buf = this.packet.encode(command);
