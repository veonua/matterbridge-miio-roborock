    ...
    Promise<smarthome.IntentFlow.QueryResponse> => {
      ...
      await Promise.all(payload.devices.map(async (google_device) => {
        ...
        try {
          ...
          deviceStates[google_device.id] = await promiseFromQuery(this.app.getDeviceManager(), queryRequest.requestId, midevice);
        }
        catch (e) {
          console.error(e);
  ...
  public executeHandler = async (executeRequest: smarthome.IntentFlow.ExecuteRequest):
    ...
    await Promise.all(command.devices.map(async (google_device) => {
      try {
        ...
        let result: IDeviceState = await promiseFromCommand(deviceManager, executeRequest.requestId,
          midevice, execution.command, (execution.params as IVacuumCommand)).then((value: IDeviceState) => {
            return midevice.afterExecute(deviceManager, requestId, execution.command, execution.params)
          }).then(()=>{ // return fresh status
            return promiseFromQuery(deviceManager, requestId, midevice, true)
          })
        ...
        console.error(e);
        executeResponse.setErrorState(google_device.id, code);
      }
...
let inFlightQuery: Record<string, Promise<IDeviceState>|null> = {};
export function promiseFromQuery(deviceManager:smarthome.DeviceManager, 
  requestId: string, 
  device: IMiDevice<any, any>, 
  noCache:boolean = false) : Promise<IDeviceState> {
  
  let inFlight = inFlightQuery[device.deviceIdString]
  ...
  inFlight = device.sendHandshake(deviceManager, requestId).then(()=>
          device.sendQuery(deviceManager, requestId))
          .finally(()=> {
            inFlightQuery[device.deviceIdString] = null
          })
  inFlightQuery[device.deviceIdString] = inFlight
  return inFlight
}

export async function promiseFromCommand(deviceManager:smarthome.DeviceManager, requestId: string, device: IMiDevice<any, any>,
        command: string, params: IVacuumCommand|IAcCommand) : Promise<IDeviceState> {
        
  await device.sendHandshake(deviceManager, requestId);
  const result = await deviceManager.send(device.makeRequest(requestId, command, params));
