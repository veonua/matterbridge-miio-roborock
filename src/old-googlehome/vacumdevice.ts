...
import { IMiDevice } from './midevice.js';
...
export class VacuumDevice extends IMiDevice<IVacuumCommand, IVacuumState> {
  ...
  onQueryResponse(resp_result: [any]): IVacuumState {
  ...
  override onResponseImpl(requestId: string, command: string, resp_result: [any]): any {
    if (resp_result[0] != 'ok') {
    ...
    switch (command) {
      ...
      case 'action.devices.commands.StartStop': {
        ...
        if (new_mode) {
          ...
          await deviceManager.send(this.makeRequest(requestId, 'action.devices.commands.SetModes', m)).then((result) => {
  ...
  resetModeIfNeed(par: IStartStop): IFanPower | null {
    ...
    if (Date.now() - this.last_mode_time < 30 * 60 * 1000) {
      // send last selected mode
  ...
  constructor(deviceId: number, token: string, customData: IVacumCustomData) {
    ...
    const default_fan_power: IFanPower = customData.fan_power;
    const segments = customData.segments;
    const zones = customData.zones;
    const targets = customData.targets;
