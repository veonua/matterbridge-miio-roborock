...
import { IMiCommand, IMiResponse } from './types.js';
...
export class Packet {
  ...
  _build_miio(data: Buffer, ts: number): Buffer {
    ...
    const cipher = createCipheriv('aes-128-cbc', this._tokenKey, this._tokenIV);

    const encrypted = Buffer.concat([cipher.update(data), cipher.update(Buffer.from('00', 'hex')), cipher.final()]);
    ...
    const digest = createHash('md5').update(header).update(this.token).update(encrypted).digest();

    return Buffer.concat([header, digest, encrypted]);
  }

  public encode(val: IMiCommand): Buffer {
    const micom = { ...{ id: this.msgCounter }, ...val };
    this.msgCounter++;

    const jrequest = JSON.stringify(micom).replace('"method":{"', '"').replace(']"}}', ']"}').replace('"},"', '","');

    console.log('encoded request ', jrequest);

    const buffer = Buffer.from(jrequest, 'utf8');
    const send_ts = Math.ceil(Date.now() / 1000) + this._stampDelta + OFFSET;
  ...
  public decode(value: string[] | undefined): IMiResponse {
    ...
    const digest = createHash('md5').update(msg.slice(0, 16)).update(this.token).update(encrypted).digest();

    const checksum = msg.slice(16, 32);
    if (!checksum.equals(digest)) {
      throw Error('invalid check sum');
    }
    const decipher = createDecipheriv('aes-128-cbc', this._tokenKey, this._tokenIV);
  ...
  get needsHandshake() {
    /*
     * Handshake if it has been longer then 10 mins since last received message
     */
    const res = Date.now() / 1000 - this.lastResponse > 10 * 60;
    if (res) {
      console.debug('need handshake lastResponse:' + this.lastResponse + ' now:' + Date.now() / 1000);
    }
    return res;
  }
}
